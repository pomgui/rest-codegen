...
export interface PiFileConfig {
    ...
    /** Only used when #beforeAll==none. Existing file will be completely overwritten */
    overwrite?: boolean;
...
export interface PiComponentConfig extends PiFileConfig, PiFileName {
     ...
     * Warning: This value will be written in the "path" section of tsconfig.json
     */
    dirAlias?: string;
}
...
export interface PiDatabasePoolConfig {
    /**
     * The library supports these two databases drivers:
     * - node-mysql
     * - node-firebird
     */
    type: 'mysql' | 'firebird';
    /** Size of the pool */
    size: number;
    /** Specific options used by the selected database driver */
    options: any;
}

export interface PiProjectConfig {
    /** Project name (overrides the value of config.projects[name]) */
    name: string;
    ...
    other: PiFileConfig;
    /**
     * If included, it creates the code to use a database pool.
     */
    databasePool?: PiDatabasePoolConfig;
}
