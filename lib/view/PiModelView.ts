...
import { PiFieldDescriptor, PiField } from 'pirest-lib';
import { misc, code } from "../tools";
...
export class PiModelFileView extends PiFileView {
    ...
    private _isInterface(view: PiTypeFileView) {
        ...
        function addProperties(type: any, requiredFields: string[]): void {
            Object.entries(type.properties)
                ...
                .forEach(def => {
                    let { property, imports } = me.createProperty(def, requiredFields, code.TAB);
    ...
    private _isEnum(view: PiTypeFileView) {
        view.enums.push({
            comment: code.asComment(this.def.description),
            name: this.name,
